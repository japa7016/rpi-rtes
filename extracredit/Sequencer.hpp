...
public:
    ...
    void startServices()
    {
    	...
    	std::sort(_services.begin(), _services.end(), [](const auto& a, const auto& b) 
	{
...
private:
	...
	static auto it = _instance->_services.begin();

	if (tick < 5)
	{
        	it = _instance->_services.begin();
	}
        
	for (auto current = it; current != _instance->_services.end(); ++current)
	{
    		if (tick % (*current)->getPeriod() == 0)
    	{
        	sem_post(&(*current)->getSemaphore());
        	syslog(LOG_INFO, "Sequencer: Service with period %u ms released at tick %d", (*current)->getPeriod(), tick);
    	}
    	if ((*current)->getPeriod() > tick)
    	{
        	it = current;
        	break;
    	}
    	if (std::next(current) == _instance->_services.end())
    	{
        	it = _instance->_services.begin();
    	}
	}
